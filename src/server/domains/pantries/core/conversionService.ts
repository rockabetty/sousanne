...
export const massSet = new Set(['gram', 'kilogram', 'ounce', 'pound', 'dash'])

export const itemSet = new Set(['piece', 'whole', 'slice', 'bun', 'egg'])
 ...
 */
export const identifyTargetUnit = function (unit: string) {
...
export const convertForPantryUpdate = function (
    ...
    switch (originalUnit) {
      ...
      */
      ...
      case 'dash':
        ...
        break
      case 'bread slice':
        pantryChange.recipe_amount = ing.recipe_amount * 1.33 // A slice of bread is a little over an ounce.
      case 'count':
        if (ing.convert_to_unit === 'count') {
          pantryChange.recipe_amount = ing.recipe_amount
        } else {
          pantryChange.recipe_amount =
            ing.average_weight * ing.multiplier * ing.recipe_amount
        }
        break
      default:
        ...
        if (volumeSet.has(recipeUnit)) {
          const amountInCups = convert(ing.recipe_amount)
          ...
          pantryChange.recipe_amount = convert(ing.recipe_amount)
            .from(unitMap[recipeUnit])
...
export const getConversionData = async function (
  ingredients: PantryIngredient[]
): Promise<ApiResponse<PantryIngredient[]>> {
  let convertibleIngredients = []
  try {
    const ingredientIDList = ingredients.map((ing) => {
      return ing.id
    })
    const ingredientsWithConversionData =
      await selectConversionData(ingredientIDList)
    for (let ing of ingredients) {
      const key = ing.id
 ...
 */
export const collapseRepeatIngredients = function (
  ingredients: PantryIngredient[]
): Map<number, PantryIngredient> {
  const ingredientMap = new Map<number, PantryIngredient>()
  for (let ingredient of ingredients) {
    const key = ingredient.ingredient_id
    if (!!key) {
      const value = ingredientMap.get(key) || {
        ingredient_id: key,
        recipe_amount: 0,
      }
...
export const convertIngredientAmounts = async function (
  ingredients: PantryIngredient[]
): Promise<PantryIngredientUpdate[]> {
  try {
