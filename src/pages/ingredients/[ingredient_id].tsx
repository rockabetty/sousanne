...
const IngredientPage = () => {
    ...
    const getIngredientData = async function () {
      try {
        ...
        const options = await axios.get(
          `/api/ingredients/${ingredient_id}/options`
        )
  ...
  return (
    <div>
      ...
      <p>
        {!!ingredient?.average_weight ? (
          <span>
            They usually weigh around {parseInt(ingredient?.average_weight)}{' '}
            ounces!
          </span>
        ) : null}
        {!!ingredient?.cup_weight ? (
          <span>
            A cup of {ingredient?.name?.toLowerCase()} tends to weigh{' '}
            {parseInt(ingredient?.cup_weight)} ounces!
          </span>
        ) : null}
      </p>
      <h2>Shelf Life</h2>
